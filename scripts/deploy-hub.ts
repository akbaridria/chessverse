...
async function main() {
  ...
  const nft = await ethers.deployContract(
    "NftHub",
    [chain.gateway, chain.gasReceiver, chain.nftContract],
    {
      gasLimit: 3000000,
    }
  );
  ...
  chain.contractAddress = tx.target as string;
  ...
  const filePath = path.join(__dirname, "../src/data/list-chains.json");
